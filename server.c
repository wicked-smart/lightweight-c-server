...
void error(unsigned short code)
{
    ...
    // render template
    char body[(strlen(template) - 2 - ((int)(code) + 1) - 2 + strlen(phrase)) * 2 + 1];
...
const char* lookup(const char* path)
{
    ...
    if (strcasecmp(foo,".css") == 0)
        return "text/css";

    else if (strcasecmp(foo,".php") == 0)
        return "text/php";

    else if (strcasecmp(foo,".html") == 0)
        return "text/html";

    else if (strcasecmp(foo,".gif") == 0)
        return "text/gif";

    else if (strcasecmp(foo,".ico") == 0)
        return "text/x-icon";


    else if (strcasecmp(foo,".jpg") == 0)
        return "text/jpeg";

    else if (strcasecmp(foo,".js") == 0)
        return "text/javascript";

    else if (strcasecmp(foo,".png") == 0)
        return "text/png";
...
bool parse(const char* line, char* abs_path, char* query)               // ********TEST AND DEBUG IT BEFORE MOVING FURTHER ******
{
    ...
    // checking for method being equal to "GET"
    char *foo1 = strchr(line,' ');



    int n = foo1 -line+1;

    char *bar = (char*)malloc(n+1);
    ...
    // checking for request_target starting from "/"
    char *foo2 = strchr(foo1 , ' ');

    n = foo2 - foo1 + 1;

    strncpy(abs_path, foo1+1,n);// storing the request_target in abs_path
    abs_path[n] = '\0';

     // storing query string
    char *query_position  = strchr(abs_path, '?');
    int len = strlen(query_position);

    strncpy(query,query_position,n);

    bar = (char*) malloc(n+1);
    strncpy(bar,foo2+1, n);
    ...
    // Checking if request_target do not contain "
    ...
    if(strncmp(foo2+1, "HTTP/1.1",8) != 0 ){
        error(505);
        return false;
    }
